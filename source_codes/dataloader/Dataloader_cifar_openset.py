...
class cifar_dataset(Dataset): 
    def __init__(self, ratio, open_noise, root_dir, transform, mode, noise_file=''): 
        ...
        if self.mode=='test':            
            test_dic = unpickle('%s/cifar10/test_batch'%root_dir)
            ...
            clean_label=[]
            for n in range(1,6):
                dpath = '%s/cifar10/data_batch_%d'%(root_dir,n)
            ...
            if open_noise=='cifar100':
                cifar100_dic = unpickle('%s/cifar100/train'%root_dir)
    ...
    def __getitem__(self, index):
        ...
        if self.mode=='train':
            ...
            return img_orig, target, clean_target, index, img_aug
        else:
            img = Image.fromarray(img)
